...
def test_normality(results, signif=0.05, verbose=True):
    ...
    # #Pinv = npl.inv(results._chol_sigma_u)
    # print("Pinv: " + str(Pinv.shape))
    # w = np.array([np.dot(Pinv, u) for u in results.resid])
    # print(w[:5].round(2))
    # print(np.dot(Pinv, results.resid.T)[:5].round(2))
    # b1 = (w ** 3).sum(0) / results.nobs
    # lam_skew = results.nobs * np.dot(b1, b1) / 6
    #
    # b2 = (w ** 4).sum(0) / results.nobs - 3
    # lam_kurt = results.nobs * np.dot(b2, b2) / 24
    #
